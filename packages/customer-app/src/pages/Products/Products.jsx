...
const Products = () => {
  ...
  const filterProducts = (categoryKey, productsList = products) => {
    ...
    switch (categoryKey) {
      ...
      case "pillow":
        filtered = productsList.filter(
          (product) => product.category === "pillow"
        );
        break;
      case "notebook":
        filtered = productsList.filter(
      ...
      case "other":
        filtered = productsList.filter(
          (product) =>
            !["mug", "tshirt", "bag", "bottle", "pillow", "notebook"].includes(
              product.category
            )
        );
        break;
            ...
            <nav className="-mb-px flex space-x-8 overflow-x-auto">
              {categories.map((category) => (
                <button
                  key={category.key}
                  onClick={() =>
                    handleCategoryChange(category.key, category.label)
                  }
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex space-x-2">
                    <button
                      onClick={() => handleViewDetails(product.id)}
                      className="flex-1 bg-blue-600 text-white py-2 px-3 rounded text-sm hover:bg-blue-700 transition-colors w-full"
                    >
                      開始製作
                    </button>
                  </div>
